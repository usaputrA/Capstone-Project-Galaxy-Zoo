---
title: "STA130H1S Capstone Project"
author: "[Keane Mohammad Aushaff], [Razan Ahsan Rifandi], [Yusuf Saputra]"
...
\[\textbf{Exploring Galaxy Properties:}\]
\[\textbf{Comparing Brightness Estimates,}\]
\[\textbf{Measuring Redshift Variability,}\]
\[\textbf{and Correlating Apparent Brightness with Distance}\]

Our three questions will be focusing on the Galaxy Zoo dataset. We will need the `arrow` library to load and read them.

```{r}
library(arrow)
```

Next, we will load in both the Galaxy Zoo tabular datasets, which contains the citizen science voting data, and the corresponding columns from the NASA-Sloan Atlas.
...
# Question 3: Depending on the galaxy's apparent brightness, does that correlate to how far away that galaxy is from us?

In this problem, we are interested in analyzing the correlation on how far galaxies are from us, or the redshift of the galaxy, with their apparent brightness. Our focus is on the variables `redshift` for the redshift of the galaxy and `sersic_nmgy_r` for the galaxy's apparent brightness. To solve this inquiry, we are using simple linear regression method.

Firstly, let's look at the glimpse of the `galaxy_zoo` data set that we would like to use.

```{r}
galaxy_zoo <- galaxy_zoo %>% glimpse()
```

Secondly, let's make a plot showing the relationship between the redshift of the galaxy as the response variable (on the y-axis) and the galaxy's apparent brightness as the explanatory variable (on the x-axis).

```{r}
galaxy_zoo %>%
  ggplot(aes(sersic_nmgy_r,
             redshift)) + geom_point(alpha = 0.4) + theme_minimal() + labs(
               title = "Scatter plot of sersic_nmgy_r and redshift")
```

Well, it appears that the scatter plot does not tell us anything about the relationship between those two variables because there are lots of outliers. So, let's focus on the values of `sersic_nmgy_r` below 20,000.

```{r}
galaxy_zoo <- galaxy_zoo %>%
  filter(sersic_nmgy_r < 20000)
```

Let's plot it once again.

```{r}
galaxy_zoo %>%
  ggplot(aes(sersic_nmgy_r, redshift)) +
  geom_point(alpha = 0.4) +
  theme_minimal() +
  labs(title = "Scatter plot of sersic_nmgy_r and redshift")
```

Even though the association between these two variables is not well modeled by a straight line, we can kind of see that they have a slightly negative correlation.

Now, we plot the association between `redshift` (on the y-axis) and `sersic_nmgy_r` (on the x-axis) by adding a line of best fit to the plot.
...
galaxy_zoo %>%
  ...
  labs(title = "Simple linear regression line of sersic_nmgy_r and redshift")
```

It seems that there is a weak negative linear association between `redshift` and `sersic_nmgy_r`. Let's now calculate the correlation between `redshift` and `sersic_nmgy_r`.

```{r}
galaxy_zoo %>%
  summarise(correlation = cor(redshift, sersic_nmgy_r))
```

Based on the correlation, it tells us that the negative correlation is close to 0. Thus, the negative linear association is weak.

It might be safe to say that if the galaxy's apparent brightness gets dimmer, then the galaxy might be far away from us, or the distance of the galaxy is more further from us, vice versa.

Additionally, we are interested in testing the null and alternative hypothesis whether the slope of the linear regression model where the galaxy's apparent brightness is predicting the redshift of the galaxy. So, let's make the simple linear regression model and state the hypotheses
\[
...
Let's fit the linear model that corresponds with the line of best fit above.

```{r}
mod <- lm(redshift ~ sersic_nmgy_r, data = galaxy_zoo)
...
By substituting the estimates to the simple linear regression model, we get:
\[
\hat{y}_i = 0.08328 - 0.00001x_i
\]

The estimate of the $\beta_0$ is $\hat{\beta}_0 = 0.08$. This means that if the galaxy's apparent brightness is $0$, the redshift of the galaxy is $0.08$.

The estimate of $\beta_1$ is $\hat{\beta}_1 = -0.00001$. This means that each additional magnitude in galaxy's apparent brightness is associated with a decrease of $0.00001$ redshift of the galaxy.

Based on the fitted regression model, the $p$-value corresponding to our hypothesis test is very small, so we have very strong evidence to reject the null hypothesis and conclude that the regression line predicting the redshift of the galaxy based on the galaxy's apparent brightness is not zero.

Let's find the $R^2$ of the model and interpret it.

```{r}
mod_summ$r.squared
```

$8$% of the variation in the redshift of the galaxy is explained by our fitted regression line.

To conclude, the redshift of the galaxy has a weakly negative correlation depending on the galaxy's apparent brightness based on the $p$-value from the fitted regression model, the correlation, and the simple linear regression line.